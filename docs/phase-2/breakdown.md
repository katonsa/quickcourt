...
| P2-03 | Owner Invitation Flow        |       P0 | Done   | P2-02               | [03-owner-invitation-flow.md](./tasks/03-owner-invitation-flow.md)                    | Super Admin can invite an already-registered user as owner; acceptance grants venue access        | Unit + DB integration                          |
...
- P2-03 is implemented by `lib/organizations/owner-invitation-service.ts`; invitation creation is Super Admin-controlled, requires an already-registered user, writes a Better Auth-compatible pending owner invitation, and acceptance delegates to the Better Auth Organization invitation acceptance path so `Member.role = "owner"` unlocks `/dashboard/venue`. Duplicate safety is enforced by `member(organizationId, userId)` uniqueness and a partial unique index for pending owner invitations per Organization/email.
